%%  preprocessing_DWS_FidA_isison.m 
...
% out = obj.preprocessing_DWS_FidA_isison(prog_dbox)
% 
% DESCRIPTION :
...
% OUTPUT :
% msg       = Error message
function msg = preprocessing_DWS_FidA_isison(obj,prog_dbox)
msg = {''};
...
for i = 1:length(obj.DWS_struct)
    ...
    try
        ...
        if study.multiplicity > 1
            ...
            % impair
            out0alb = out0a;
            fids0alb = out0alb.fids.*repmat(exp(-tt*pi*dws_param.LBall).',1,out0a.averages);
            out0alb.fids = fids0alb;
            out0alb.specs = fftshift(fft(out0alb.fids.',[],2),2).';
            % pair
            out0blb = out0b;
            fids0blb = out0blb.fids.*repmat(exp(-tt*pi*dws_param.LBall).',1,out0b.averages);
            ...
            if dws_param.FidA_align_avg
                ...
                out1a = out1alb;
                fids1a = out1alb.fids.*repmat(exp(tt*pi*dws_param.LBall).',1,out1alb.averages);
                out1a.fids = fids1a;
                out1a.specs = fftshift(fft(out1a.fids.',[],2),2).';
                out1b = out1blb;
                fids1b = out1blb.fids.*repmat(exp(tt*pi*dws_param.LBall).',1,out1blb.averages);
                out1b.fids = fids1b;
            ...
            if dws_param.FidA_rm_badavg
                [out2a,metrica,badAveragesa] = obj.rmbadaverages_FidA(out1a,dws_param.sd_thresh,'f'); %performs 10Hz LB inside
                [out2b,metricb,badAveragesb] = obj.rmbadaverages_FidA(out1b,dws_param.sd_thresh,'f'); %performs 10Hz LB inside
    
                %apply LB
                out2alb = out2a;
                fids2alb = out2a.fids.*repmat(exp(-tt*pi*dws_param.LBall).',1,out2a.averages);
