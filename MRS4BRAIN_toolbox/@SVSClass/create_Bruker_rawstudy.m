...
for i = 1:length(obj.SVS_struct)
    prog_dbox.Message = ['Create raw study for : ',obj.SVS_struct(i).exp_name];
    prog_dbox.Value = i/(length(obj.SVS_struct)+1);
    study = struct;
    folderexp = obj.SVS_struct(i).folder;
    expnb = obj.SVS_struct(i).expnb;
    ...
    obj.SVS_struct(i).raw_study = raw_study;
